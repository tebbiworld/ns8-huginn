...
# Reuse existing nodebuilder-huginn container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-huginn; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-huginn -v "${PWD}:/usr/src:Z" docker.io/library/node:24.16.0-slim
fi
...
# Setup the entrypoint, reserve one TCP port (Huginn web), declare the runtime
# images and mark the module as rootless.
buildah config --entrypoint=/ \
    --label="org.nethserver.authorizations=traefik@node:routeadm" \
    --label="org.nethserver.tcp-ports-demand=1" \
    --label="org.nethserver.rootfull=0" \
    --label="org.nethserver.images=${huginn_image} ${mariadb_image}" \
    "${container}"
# Commit the image
