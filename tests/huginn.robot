...
*** Test Cases ***
Check if huginn is installed correctly
...
Check if huginn can be configured
    ${rc} =    Execute Command    api-cli run module/${module_id}/configure-module --data '{}'
    ...    return_rc=True  return_stdout=False
    Should Be Equal As Integers    ${rc}  0

Check if huginn works as expected
    ${rc} =    Execute Command    curl -f http://127.0.0.1/huginn/
    ...    return_rc=True  return_stdout=False
    Should Be Equal As Integers    ${rc}  0

Check if huginn is removed correctly
